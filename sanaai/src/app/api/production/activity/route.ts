// app/api/production/activity-logs/route.ts
//
// 🆕 ده الفيتشر الجديد (شفتات مرتبطة بالطلبات) — اتنقل هنا من
// /api/production/logs عشان محدش يتعارض مع فيتشر production_logs القديم
// اللي page.tsx الحالي شغال عليه.
//
// 🔒 إضافة أمان مهمة كانت ناقصة: قبل كده الكود كان بيثق إن machine_id،
// order_id، employee_id الجايين في الـ body بتوع نفس الـ tenant من غير أي
// تحقق فعلي — ده ثغرة عزل بيانات (ممكن حد يبعت order_id بتاع مصنع تاني).
// دلوقتي التلاتة بيتفحصوا فعليًا قبل أي insert/update، بنفس المبدأ اللي كان
// متبع في production_logs القديم للمكينة.
...
async function assertBelongsToTenant(
  table: 'machines' | 'orders' | 'employees',
  id: string,
  tenantId: string,
...
// GET /api/production/activity-logs — سجل نشاط المكن، بفلاتر اختيارية:
...
// POST /api/production/activity-logs — إضافة تسجيل جديد لنشاط مكينة
export async function POST(request: NextRequest) {
  try {
    ...
    // 🔒 فحص الملكية الثلاثي — الأهم في المراجعة دي
    await assertBelongsToTenant(
      'machines',
      validated.machine_id,
      user.tenantId,
      'المكينة غير موجودة أو لا تتبع مصنعك'
    )
    await assertBelongsToTenant(
      'orders',
      validated.order_id,
