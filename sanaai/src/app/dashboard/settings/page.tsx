...
export default function SettingsPage() {
  ...
  const [notifSettings, setNotifSettings] = useState({
    order_delayed: true, order_ready: true, quality_failed: true, invoice_due: true, complaint_new: true, trial_ending: true,
  })
  ...
  async function saveTenant() {
    if (!tenant.id) { alert('خطأ: معرف الشركة غير موجود'); return }
    setLoading(true)
    try {
  ...
  if (pageLoading) return <div className="flex items-center justify-center min-h-screen bg-[#080C12] text-white"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-amber-500"></div></div>
    ...
    <div className="p-6 min-h-screen" dir="rtl" style={{ fontFamily: "'Cairo', sans-serif" }}>
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-1">
          {[{ key: 'company', label: '🏢 بيانات الشركة' }, { key: 'users', label: '👥 المستخدمون' }, { key: 'notifications', label: '🔔 الإشعارات' }, { key: 'defaults', label: '⚙️ البيانات الافتراضية' }].map(t => (
            <button key={t.key} onClick={() => setActiveTab(t.key as any)} className={`px-4 py-2 rounded-xl text-sm font-bold transition ${activeTab === t.key ? 'bg-amber-500 text-black' : 'bg-[#111927] text-gray-400 border border-white/5 hover:text-white'}`}>{t.label}</button>
          ))}
        </div>
        ...
        {activeTab === 'company' && (
          <div className="bg-[#111927] rounded-2xl border border-white/5 p-6 space-y-4">
            ...
            <div className="flex items-center gap-3 pt-2">
              <button onClick={saveTenant} disabled={loading} className="px-6 py-2.5 bg-amber-500 text-black font-bold rounded-xl hover:bg-amber-400 transition disabled:opacity-50">{loading ? 'جاري الحفظ...' : '💾 حفظ التغييرات'}</button>
        ...
        {activeTab === 'users' && (
          <div className="space-y-3">
            {users.map(user => (
              <div key={user.id} className="bg-[#111927] rounded-2xl border border-white/5 p-4 flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-amber-500/20 flex items-center justify-center text-amber-400 font-bold text-sm flex-shrink-0">{user.full_name?.charAt(0) || '?'}</div>
                <div className="flex-1 min-w-0">
                  <p className="text-white font-bold text-sm truncate">{user.full_name}</p>
                  <p className="text-gray-500 text-xs truncate">{user.email}</p>
                </div>
                <select value={user.role} onChange={e => changeUserRole(user.id, e.target.value)} className="bg-[#0D1B2A] border border-white/10 rounded-lg px-2 py-1.5 text-xs text-white focus:outline-none focus:border-amber-500/50">
                  {Object.entries(ROLES).map(([val, label]) => (<option key={val} value={val}>{label}</option>))}
                </select>
                <button onClick={() => toggleUserActive(user.id, user.is_active)} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition ${user.is_active ? 'bg-green-500/20 text-green-400 hover:bg-red-500/20 hover:text-red-400' : 'bg-red-500/20 text-red-400 hover:bg-green-500/20 hover:text-green-400'}`}>{user.is_active ? '✅ نشط' : '❌ موقوف'}</button>
              </div>
            ))}
          </div>
        )}
        ...
        {activeTab === 'notifications' && (
          <div className="bg-[#111927] rounded-2xl border border-white/5 p-6 space-y-4">
            ...
            {[ { key: 'order_delayed', label: 'طلب متأخر', desc: 'إشعار عند تأخر أي طلب عن موعد التسليم' }, { key: 'order_ready', label: 'طلب جاهز', desc: 'إشعار عند اكتمال الطلب وجاهزيته للشحن' }, { key: 'quality_failed', label: 'فشل فحص الجودة', desc: 'إشعار عند رسوب أي طلب في فحص الجودة' }, { key: 'invoice_due', label: 'فاتورة مستحقة', desc: 'إشعار عند اقتراب موعد سداد فاتورة' }, { key: 'complaint_new', label: 'شكوى جديدة', desc: 'إشعار عند ورود شكوى جديدة من عميل' }, { key: 'trial_ending', label: 'انتهاء التجربة', desc: 'إشعار قبل انتهاء فترة التجربة المجانية' }].map(item => (
              <div key={item.key} className="flex items-center justify-between py-3 border-b border-white/5 last:border-0">
                <div><p className="text-white text-sm font-bold">{item.label}</p><p className="text-gray-500 text-xs">{item.desc}</p></div>
                <button onClick={() => setNotifSettings(s => ({ ...s, [item.key]: !s[item.key as keyof typeof s] }))} className={`w-12 h-6 rounded-full transition-colors relative ${notifSettings[item.key as keyof typeof notifSettings] ? 'bg-amber-500' : 'bg-gray-700'}`}><span className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${notifSettings[item.key as keyof typeof notifSettings] ? 'right-1' : 'left-1'}`} /></button>
              </div>
            ))}
          </div>
        )}
        ...
        {activeTab === 'defaults' && (
          <div className="space-y-4">
            <div className="bg-[#111927] rounded-2xl border border-white/5 p-6">
              ...
              <div className="flex flex-wrap gap-2">{sectors.map(s => (<span key={s} className="px-3 py-1.5 bg-[#0D1B2A] border border-white/10 rounded-lg text-sm text-white">{s}</span>))}</div>
            </div>
            <div className="bg-[#111927] rounded-2xl border border-white/5 p-6">
