...
// 1. تعريف الأنواع بدقة لتجنب أخطاء TypeScript
type InventoryItem = {
  id: string
  name: string
  unit: string
  current_stock?: number // جعلناه اختياري لأن التحميل قد لا يشمله دائماً
}
...
// تعديل النوع ليتناسب مع بيانات Supabase عند عمل Join
type PurchaseOrder = {
  id: string
  po_number: string
  status: string
  ...
  suppliers: { name: string } | null // الجداول المرتبطة تكون كائن أو null
...
export default function ProcurementPage() {
    ...
    async function load() {
      ...
      try {
        ...
        const [poRes, supRes, invRes] = await Promise.all([
          ...
          supabase.from('inventory').select('id, name, unit, current_stock')
        ])
  ...
  const addItem = () => {
    setItems(prev => [...prev, { inventory_id: '', name: '', qty: 1, price: 0 }])
  }
  ...
  const updateItem = (index: number, key: keyof POItem, value: any) => {
  ...
  async function handleSavePO() {
    if (!form.supplier_id || items.length === 0) {
      alert('يرجى اختيار المورد وإضافة صنف واحد على الأقل')
      return
    }
    ...
    try {
      ...
      // تحويل البيانات ليتطابق مع نوع PurchaseOrder قبل الإضافة للـ State
      const newPo = { 
        ...po, 
      ...
      setShowForm(false)
      setItems([])
      setForm({ supplier_id: '', order_id: '', po_number: '', expected_date: '' })
    } catch (err: any) {
  ...
  async function receivePO(poId: string) {
    ...
    try {
      ...
      if (itemsData) {
        for (const item of itemsData) {
          if (item.inventory_id) {
            ...
            // حل مشكلة العملية الحسابية: التأكد من وجود رقم
      ...
      alert('تم استلام الشحنة وتحديث المخزون بنجاح!')
    } catch (err: any) {
      alert('خطأ في الاستلام: ' + err.message)
    } finally {
      setSaving(null)
    }
  }
        ...
        <div className="bg-[#111927] rounded-2xl border border-amber-500/20 p-6 mb-6 shadow-2xl">
          ...
          <div className="space-y-3 mb-6">
            <div className="flex justify-between items-center mb-2">
              ...
              <button onClick={addItem} className="text-xs bg-amber-500/20 text-amber-400 px-2 py-1 rounded border border-amber-500/30 hover:bg-amber-500/30 transition">➕ إضافة صنف</button>
            </div>
            {items.map((item, i) => (
              <div key={i} className="grid grid-cols-4 gap-2 bg-white/5 p-2 rounded-lg border border-white/5">
                ...
                <input type="number" value={item.qty} onChange={e => updateItem(i, 'qty', Number(e.target.value))} placeholder="كمية" className="bg-[#0D1B2A] border border-white/10 rounded px-2 py-1 text-xs text-white" />
                <input type="number" value={item.price} onChange={e => updateItem(i, 'price', Number(e.target.value))} placeholder="سعر" className="bg-[#0D1B2A] border border-white/10 rounded px-2 py-1 text-xs text-white" />
                <button onClick={() => removeItem(i)} className="text-red-400 text-xs">✕</button>
              </div>
            ))}
          </div>
          ...
          <div className="flex justify-end gap-3">
            ...
            <button onClick={handleSavePO} disabled={!!saving} className="px-6 py-2 bg-amber-500 text-black font-bold rounded-lg text-sm hover:bg-amber-400 transition disabled:opacity-50">
            ...
            <div key={po.id} className="bg-[#111927] rounded-2xl border border-white/5 p-5 flex items-center justify-between">
                ...
                <div className="flex flex-col">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded-full border ${
                      po.status === 'تم الاستلام' ? 'bg-green-500/20 text-green-400 border-green-500/30' : 
                      po.status === 'قيد الانتظار' ? 'bg-amber-500/20 text-amber-400 border-amber-500/30' : 'bg-gray-500/20 text-gray-400 border-gray-500/30'
                    }`}>{po.status}</span>
                  </div>
              ...
              <div className="flex items-center gap-3">
                {po.status !== 'تم الاستلام' && (
                  <button onClick={() => receivePO(po.id)} disabled={saving === po.id}
                    className="flex items-center gap-2 px-4 py-2 bg-green-600/20 text-green-400 border border-green-600/30 rounded-xl text-xs font-bold hover:bg-green-600/30 transition">
                    {saving === po.id ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle size={14} />}
                    تأكيد الاستلام
                  </button>
                )}
