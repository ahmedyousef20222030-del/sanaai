...
// ✅ تحديث الأنواع لتطابق السكيما v1.1
...
type OrderItem = {
  inventory_id: string // ✅ تغيير من product_id إلى inventory_id
  product_name: string
  size: string
  qty: number
  unit_price: number
  stock_qty: number
  status: 'متاح من المخزون' | 'مطلوب تصنيع' | 'مطلوب شراء' // ✅ تطابق مع fulfillment_type
}

export default function NewOrderPage() {
  const router = useRouter()
  const [clients, setClients]   = useState<Client[]>([])
  const [inventory, setInventory] = useState<InventoryItem[]>([]) // ✅ تغيير الاسم
  ...
  useEffect(() => {
    // ✅ جلب البيانات من الجداول الصحيحة
    ...
    setItems(prev => prev.map((item, idx) => {
      ...
      if (key === 'inventory_id') {
        ...
        if (prod) {
          ...
          // ✅ تحديث منطق القرار بناءً على المخزون
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // 2. إضافة الأصناف إلى جدول order_items (بدلاً من JSON)
      if (items.length > 0) {
        const itemsToInsert = items.map(item => ({
          ...
          fulfillment_type: item.status, // ✅ حفظ قرار مدير التنفيذ
      ...
      // 3. خصم الكميات من المخزون (Inventory)
  ...
  // ── بقية الدوال (PDF, Files) تبقى كما هي مع تعديل مسميات الحقول ──
