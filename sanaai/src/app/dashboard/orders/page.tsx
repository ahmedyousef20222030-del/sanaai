        ...
        <div className="flex items-center gap-4">
          ...
          <div className="mr-6 px-4 py-2 bg-amber-500/10 border border-amber-500/20 rounded-xl">
            ...
            <span className="text-amber-400 font-bold text-sm">
              {filtered.reduce((acc, curr) => acc + (curr.total_amount || 0), 0).toLocaleString('ar-EG')} ج.م
            </span>
          </div>
        </div>
        <button onClick={() => router.push('/dashboard/orders/new')} className="px-5 py-2.5 bg-amber-500 text-black font-bold rounded-xl hover:bg-amber-400 transition text-sm">➕ طلب جديد</button>
      </div>
